...
export function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  return (
    <article className={cn("group flex gap-3", isUser && "justify-end")}>
      {!isUser && (
        <div className="mt-1 grid h-9 w-9 shrink-0 place-items-center rounded-full bg-ink text-surface">
      ...
      <div className={cn("max-w-[920px] flex-1", isUser && "flex max-w-[760px] justify-end")}>
        <div
          className={cn(
            ...
            isUser
              ? "border-ink/80 bg-ink text-surface"
        ...
        >
          <div className="flex items-start justify-between gap-3">
            ...
            {!isUser && message.status !== "streaming" && (
              <button
                type="button"
                onClick={copy}
                className="shrink-0 rounded-full p-1.5 text-ink/45 opacity-0 outline-none transition hover:bg-ink/5 hover:text-ink focus:opacity-100 focus:ring-2 focus:ring-primary/40 group-hover:opacity-100"
                title="复制"
                aria-label="复制"
              >
          ...
          <div
            className={cn(
              "mt-3 text-xs",
              isUser ? "text-surface/55" : "text-ink/45",
            )}
          >
