...
class MetaKnowledgeService:
    ...
    async def _save_tables_to_meta_db(
        ...
        async with self.meta_mysql_repository.session.begin():
            # 先清除旧数据，保证脚本可重复执行
            await self.meta_mysql_repository.delete_column_infos()
            await self.meta_mysql_repository.delete_table_infos()
    ...
    async def _save_metrics_to_meta_db(
        ...
        async with self.meta_mysql_repository.session.begin():
            # 先清除旧数据，保证脚本可重复执行
            await self.meta_mysql_repository.delete_column_metrics()
            await self.meta_mysql_repository.delete_metric_infos()
